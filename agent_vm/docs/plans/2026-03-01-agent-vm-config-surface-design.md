...
6. **Host Docker services are accessed only through explicit `tcp.hosts` mappings** (formalized in [Gondolin PR #61](https://github.com/earendil-works/gondolin/pull/61), now merged). No generic TCP forwarding subsystem. Mapped TCP flows bypass HTTP hooks/secret substitution — they are raw tunnels to configured upstream targets.
...
1. Non-HTTP host services use only Gondolin `tcp.hosts` mappings (formalized in [Gondolin PR #61](https://github.com/earendil-works/gondolin/pull/61), now merged).
...
6. **Mapped TCP flows are raw tunnels** — they bypass Gondolin's HTTP hook pipeline and HTTP secret placeholder substitution entirely. Only HTTP/TLS-mediated flows receive content-aware policy enforcement. See Gondolin's [Egress Capability Matrix](https://earendil-works.github.io/gondolin/security/#egress-capability-matrix) for the full trust boundary breakdown.
7. **Mapping keys are always `HOST:PORT`** — `buildTcpHostsRecord()` produces port-specific keys (`pg.vm.host:5432`), not host-only keys. Gondolin supports host-only keys (matching all ports), but we deliberately use port-specific mappings for narrower security surface.
8. **DNS configuration is enforced by Gondolin** — `tcp.hosts` requires `dns.mode: "synthetic"` and `dns.syntheticHostMapping: "per-host"`. Gondolin validates this at VM construction time via `assertTcpDnsConfig()` and throws if violated. Our vm-adapter sets these values when `tcpHosts` is non-empty, matching Gondolin's CLI auto-selection behavior.
...
6. TCP mapping DNS enforcement:
   - non-empty `tcpHosts` → VM.create receives `dns: { mode: 'synthetic', syntheticHostMapping: 'per-host' }` and `tcp: { hosts: ... }`
   - `buildTcpHostsRecord()` always produces `HOST:PORT` keys (port-specific format)
   - Gondolin's `assertTcpDnsConfig()` validates DNS config at construction time; missing DNS options causes a throw
...
- **SDK VM API** (VM.create, sandbox.imagePath): https://earendil-works.github.io/gondolin/sdk-vm/
- **SDK Network** (mapped TCP egress, `tcp.hosts` API, DNS requirements): https://earendil-works.github.io/gondolin/sdk-network/ — see "Mapped TCP Egress (Optional)" section
- **Security** (egress capability matrix, trust boundaries for HTTP/TLS vs mapped TCP vs SSH): https://earendil-works.github.io/gondolin/security/
- **Network Stack** (TCP flow classification, mapped TCP checked first then HTTP/TLS/SSH): https://earendil-works.github.io/gondolin/network/
