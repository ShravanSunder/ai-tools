...
Codex skills are delivered by installed Codex plugins. Do not add runtime sync scripts or symlink-based delivery into consumer projects. Plugin installation remains the delivery mechanism.
...
Home: `~/dev/memory-logs/skills/` (clone of https://github.com/ShravanSunder/memory-logs)
...
For execution ownership, load `manage-agents` and follow it decisively. Main uses the owner's model choice, normally a daily driver, remains the default user conversation, and always authors governing Requirements, Specification, Program Design, diagrams, and implementation plans. After a ready Main-authored plan, the assigned persistent implementation Sidekick executes the bounded implementation and associated proof directly by default; direct contact with it is available only when the user chooses it. Material design/plan or integration questions return to Main with evidence, while Main does not relay every internal progress turn. Delegate only for a concrete independent-work, expertise, or disposable-output benefit that exceeds coordination cost. Standalone prescribed Git, PR, build, test, and watch procedures use Operators; implementation-associated proof stays with its executor. PR wrap-up may stay with the assigned implementation Sidekick or run as a prescribed Operator procedure; Main retains acceptance and the user retains merge authority.
...
Design-view ownership follows the same split: `shared-references/diagram-rendering-and-fallbacks.md` owns runtime medium selection, fallback, semantic preservation, and visual checks; `shared-references/generated-document-visuals.md` owns conditional Image Gen capability, durable asset/embed and pixel/preview procedure; each design skill's `SKILL.md` owns its view predicates, required semantic fields and main-authored visual meaning; `docs/diagram-vocabulary.md` is the maintainer index of tokens, altitude, owners, and consumers.
...
Skills are layered by the role they play: orchestrators run a whole job; phases are its steps; practices (`practices-collaboration` < `practices-show-me-your-work` < `manage-agents` < `practices-research`) are always on, with `ops-*` and `presentation-*` at the same level; tool manuals such as `agent-collaboration` cover only how to call a tool. Within a layer, skills may name each other freely, cycles included. No skill names a skill in a higher layer: phases never name an orchestrator, and practices, tool manuals, and shared references never name phases or orchestrators except as consumer declarations or data values. Work sent upward returns a token from `shared-references/phase-return-tokens.md`, which orchestrators map to the next owner; mapping never widens the requested task. Phase skills open or resume the trace through `practices-show-me-your-work` at entry and checkpoint at completion. Devfiles is the session root outside the layers. The governing spec is `docs/wip/skills-authoring/2026-09-25-capability-revamp/proposal.md`.
...
| spec-program-review | `plugins/shravan-dev-workflow/skills/spec-program-review/` | Independent 🔎 Review Sidekick walks mode-complete, sequential seam passes, dispel, proof challenge, and predicate-selected focused checks; rails-anchored reduction, no editing or acceptance, bounded review rounds |
...
| practices-research | `plugins/shravan-dev-workflow/skills/practices-research/` | Practice: evidence gathering for local code/docs, prior art, current web/docs, Reader, memory, and session-log research with ordered source classes, `tmp/practices-research/` ledgers, and a return token for the next owner |
| manage-agents | `plugins/shravan-dev-workflow/skills/manage-agents/` | Practice: authority, roles and titles, selection by table (`references/model-catalog.md`), runtime (native, agent-router, legacy ACPX), handoff, and verification for advisors, sidekicks, workers, operators, and subagents |
...
| plan-implementation | `plugins/shravan-dev-workflow/skills/plan-implementation/` | Main-authored translation of one current ready Requirements/Specification/Program Design set into a repo-grounded proof-bearing plan |
| plan-improve-repo | `plugins/shravan-dev-workflow/skills/plan-improve-repo/` | Audit a repository for high-leverage improvements; the main writes or validates canonical plans only after planning admission |
...
| implementation-review | `plugins/shravan-dev-workflow/skills/implementation-review/` | Independent 🔎 Review Sidekick reads the whole map and walks spec-compliance, sequential overlapping chunk passes, proof challenge, dispel, and focused checks; rails-anchored reduction and cause-based routing without editing |
...
| skills-creation | `plugins/shravan-dev-workflow/skills/skills-creation/` | Main-authored skill proposal/spec/plan with delegated accepted-run implementation, proof, main assessment, then independent review |
...
| agent-collaboration | `plugins/agent-router/skills/agent-collaboration/` | Tool manual for the agent-collaboration CLI and agent-router MCP: identity, sessions, boards, messages, listen/wait, wakes, schedules, uncertain-mutation recovery (Codex, Claude Code, and Cursor via `agent-router`) |
