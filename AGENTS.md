...
For execution ownership, load `manage-agents` and follow it decisively. Main always authors governing Requirements, Specification, Program Design, diagrams, and implementation plans. After a ready Main-authored plan, the assigned persistent implementation Sidekick becomes the normal execution contact for its bounded assignment, executes the implementation and associated proof directly by default, and escalates material design/plan or integration questions with evidence. Delegate only for a concrete independent-work, expertise, or disposable-output benefit that exceeds coordination cost. Standalone prescribed Git, PR, build, test, and watch procedures use Operators; implementation-associated proof stays with its executor.
