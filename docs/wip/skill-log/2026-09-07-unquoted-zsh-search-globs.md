...
2026-09-10 recurrence: primary Rust source investigation used unmatched guessed burn-rate, selector, and repository path globs. zsh aborted those reads; root then searched the crate roots with rg and used the returned paths. No mutation or data loss. This remains an agent command-construction failure, not a confirmed skill defect.

2026-09-12 recurrence during Swift lifecycle work: primary reads guessed a Remote runtime folder glob and a test-target folder glob. zsh rejected both before ripgrep ran; subsequent whole-root rg inventories located the files under Git and the actual architecture-test target. Other guessed exact paths also failed. No mutations resulted. The same inventory-first correction remains required; no skill implementation cause is established.

2026-09-13 IPC implementation recurrence: the primary attempted several nonexistent exact filenames (including IPC registry, Sessions transition/intent, and an IPCAdapters directory) before consuming `rg --files` results. Reads returned file-not-found with no source mutation. Actual owners were then located under the existing IPCComposition/Runtime files. This remains a primary-agent source-discovery failure; inventory-first is the correction, and no skill implementation defect is established.
