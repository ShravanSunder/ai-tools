...
This changeset updates the four primary runtime skills, the scoped `docs-maintain` trigger boundary needed to route semantic authoring and independent review correctly, their shared diagram support, the maintainer index, focused static checks, deferred pressure-scenario files, plugin metadata, and changelog. It also applies the smallest direct-consumer cutover needed for planning and handoff skills to consume the new current-reviewed-pair result without artifact digests; it does not redesign their planning or handoff workflows. It does not create `orchestrator-spec`, retire `skill-audit`, modify retired skills, or redesign the wider implementation workflow. The generic `manage-agents` resource budget remains a separate focused follow-up; this changeset closes the reproduced failure inside `spec-program-review` by using mode-first serialized dispatch and no automatic reviewer multiplication.
