...
| A native page need not cover every role or model category; it lists only what that host offers and the owner wants. Owner mapping, 2026-09-25: Worker: Claude Code native Opus low; Cursor native Grok 4.6 medium or Opus low; Codex native Luna or Sol. Operator: Luna (Codex native, otherwise agent-router). Cursor: other Opus levels and Fable only on request. When a host's native page has no allowed id for the role's catalog model (Cursor has no Luna, Sol, or Astra), do not substitute a native stand-in: launch that 🛠️ Worker or 🔧 Operator through agent-router with the catalog model (Luna for Workhorse work), as a bounded assignment that finishes after acceptance | Owner, 2026-09-25: "for cursor ... fable or opus is only used on request; if it doesn't have native we don't use it because it can use agent router and luna". Replaces "an unavailable native lineup is a reported gap". |
| Model ids live outside skills: skills state policy as lineage + effort + route per host; an optional machine map (`~/.config/agent-context/model-map.md`, devfiles-managed, home and work variants) names exact ids; otherwise agents resolve from the host's live list using only the current generation of the named lineage (no older generations, no `-fast`), else agent-router with Luna | Owner, 2026-09-25: work Cursor has more models than home Cursor; exact ids in public skills break across machines. Supersedes exact-id tables on native and ACPX pages. |
| DAG between levels only: within a level skills may reference each other freely (phase to phase included); no skill names a skill in a higher level, so phases never mention orchestrators and practices, tools, and shared references never mention phases or orchestrators; upward routing uses a return token | Owner, 2026-09-25: "dags between levels only, so children of orchestrator should not mention orchestrator". Replaces the same-layer acyclic rule and the phase-to-phase token conversion. |
