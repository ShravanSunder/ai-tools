...
- Syncs the vendored `agent-collaboration` skill to codex-router v0.1.28 (`788be52`) through `scripts/sync-skills.py` with a full-SHA pin.
...
- Validation: `claude plugin validate .` passed; `scripts/sync-skills.py --check` verified the pin at 6 files; `diff -r` against the source tree is empty.
