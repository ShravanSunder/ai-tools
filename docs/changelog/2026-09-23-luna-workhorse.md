...
- `manage-agents` uses Workhorse and Daily driver tiers. Guidance and Architectural span remain task signals. Clear work off the critical path fits Workhorse; live conversation and work blocking someone now use a daily driver.
...
- `implementation-pr-wrapup` uses Workhorse 🛠️ Workers for description drafts and Workhorse Operators for monitors off the critical path. Pressure fixtures cover the new selection and contact boundaries.
