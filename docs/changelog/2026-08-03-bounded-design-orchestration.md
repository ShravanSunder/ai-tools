...
- Allows bounded pre-review recovery, one pair review, and one post-review correction pass. A pair-review correction enters `post-review` before invoking its semantic owner, and a semantic correction never triggers an automatic second review.
...
- Permanent Luna-high/Terra-medium pressure scenarios for the four direct phase handoffs, exact pathfinding returns and stops, fresh and resumed routing, invalid routes, bounded pre-review recovery, post-review correction entry, and the stale-review stop.
...
- `pnpm --dir tests/skills exec vitest run evals --config vitest.config.ts -t orchestrator-design-enters-post-review-correction` — passed: 1 test, 129 skipped; all four deterministic evaluators and the Terra semantic evaluator scored `1.00`.
