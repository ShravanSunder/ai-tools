...
- New skill `discuss-pathfinding`: extract understanding that lives only in someone's head or an unmade decision — requirements, tacit process knowledge, domain terms, design decisions. Spine: name the destination, depth, and out-of-scope → classify before asking (observable → bounded read; broad → research-swarm; judgment/tacit → the user) → hypothesize with confidence first → batched questions, one axis at a time by decision leverage → challenge as you go → write decision, process, and glossary records the moment they crystallize → provenance-split validation with an explicit-resolution bar (a concrete selection, constraint, example, or correction resolves; generic assent never does) → private predictive stop test with a provisional-stop shape. Session scales to the user's stated time budget (quick/standard/deep); three layers stay separate (private working state, conversational surface, durable records). Bright lines: refuses non-interactive contexts; chat-only changes where records live, never whether they exist.
...
- `skills-creation` step 2 now sources ground truth before design with two independent calls: `discuss-pathfinding` for work that lives in people (returns validated understanding and records), `research-swarm` for work that lives in artifacts (returns the evidence consumed); main path, depth, and implementation consume the applicable returns.
