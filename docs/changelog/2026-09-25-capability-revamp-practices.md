...
- `agent-collaboration` is now a tool manual only: calls, identity, seat values, listen/wait, wakes, schedules, and uncertain-mutation recovery. Coordination policy, seat meanings, and resolution rules moved out. The vendored copy awaits the matching codex-router change and a re-pin.
