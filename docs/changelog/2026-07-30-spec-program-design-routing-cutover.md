...
- Applies the same planning-admission boundary to `plan-improve-repo`: it may vet findings before admission, but writes or marks an executable plan `ready` only from a current exact-digest pair-ready result with current required local review coverage or a positively proven implementation-mechanics-only classification. Direct work on one named runtime skill package routes through `skills-creation`.
...
- New skill trees and OpenAI metadata for `spec-design`, `program-design`, and `spec-program-review`.
- `plan-creation-swarm`, `spec-handoff`, plugin README, and trigger-eval routing.
- `plan-improve-repo` entry routing, executable-plan admission, plan template, validation checklist, and OpenAI metadata.
...
- Fresh implementation review completed under `skills-creation` with read-only OpenAI `gpt-5.6-sol` at adapter-confirmed `reasoning_effort=xhigh`. The final focused receipt matched the corrected tracked-diff and status-set digests and returned no surviving findings.
...
- Pressure testing and behavioral smoke execution were deferred by user direction. No behavior proof is claimed. The unexecuted smoke matrix is recorded in [references/shravan-dev-workflow-smoke.md](references/shravan-dev-workflow-smoke.md).
