...
Pattern: mockup at top, `▲` under each element, `│` continuing down, `└─` branching to label text. Each callout is one line. Read top-down.
...
└──────────────────────────────┘
 ...
 Spec:
   ...
   background:         AppStyle.fillMuted (rest) → fillPressed (active)
   shape:              Capsule
```
...
Double border (`╔═╗ ║ ╚═╝`) wraps the entire proposal. Title on top row with `║ Title  ║`. Separator `╠═╣` between title and body. Use for design proposals that need to stand apart from surrounding chat.
