...
6. Report:
   ...
   - recommended next skill: `spec-design` for missing Why/What, `program-design` for missing How, `spec-program-review` for a complete but unreviewed/stale three-artifact design, `docs-maintain` for artifact lifecycle work, or an explicit blocked planning handoff when current three-artifact-design coverage is `ready` but no planning route is shipped
