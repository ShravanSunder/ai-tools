...
6. Report:
   ...
   - recommended next skill: `spec-design` for missing Why/What, `program-design` for missing How, `spec-program-review` for a complete but unreviewed/stale pair, `plan-creation-swarm` only when semantically current pair-ready coverage of both current artifacts is proven, or `docs-maintain` for artifact lifecycle work
