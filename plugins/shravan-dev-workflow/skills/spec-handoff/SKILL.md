...
- Do not create an implementation plan here. Recommend `plan-creation-swarm` for design-bearing work only when the packet proves a pair-mode `spec-program-review` result is `ready` and semantically current for the specification and program design. Missing structural How routes to `program-design`; a complete but unreviewed or semantically stale pair routes to `spec-program-review`; missing Why/What routes to `spec-design`.
...
6. Report:
   ...
   - recommended next skill: `spec-design` for missing Why/What, `program-design` for missing How, `spec-program-review` for a complete but unreviewed/semantically stale pair, `plan-creation-swarm` only when semantically current pair readiness is proven, or `docs-maintain` for artifact lifecycle work
...
- Routing a handoff to planning without a pair-mode `ready` review result whose semantic coverage is current for the specification and program design.
