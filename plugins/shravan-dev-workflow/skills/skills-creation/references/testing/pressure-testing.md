...
`pnpm --dir tests/skills run test:evals` runs the fast Vitest suite; prefix it with `SKILL_PRESSURE_SCENARIO=<id>` to run one scenario. Scenario file format and the runner's grader-only checks are owned by `tests/skills/README.md` — cite it, do not restate its checklist here.
