...
Maximum authority: read-only inspection of in-scope wording, failure forms, and completion criteria. Return candidate findings; the 🔎 Review Sidekick owns wording edits, gates, and the final verdict.
...
Output focus: Use the already-loaded Check Finding and Per-Check Result shapes from `lane-schema.md`. Each finding names the targeted failure, the current form, the proposed form, and how a reviewer would tell it worked.
