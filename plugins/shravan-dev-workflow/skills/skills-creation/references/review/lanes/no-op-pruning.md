...
Maximum authority: read-only classification of in-scope sentences and deletion recommendations. Return candidate findings; the 🔎 Review Sidekick owns edits and the final verdict.
...
Output focus: Use the already-loaded Check Finding and Per-Check Result shapes from `lane-schema.md`. Each finding quotes the sentence with its `path:line` and states what behavior is unchanged by its removal.
