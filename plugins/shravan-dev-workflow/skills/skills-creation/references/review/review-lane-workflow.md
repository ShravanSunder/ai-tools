...
This reference owns the dispatch and receipt lifecycle shared by spec and implementation review lanes. The coordinator commissions the persistent independent review lead; the lead runs this workflow and reduces lanes. Stage references select lanes, lane references own their missions, and `manage-agents` owns reviewer runtime policy.
...
The review lead collects every receipt, verifies candidate findings against source, merges duplicates, resolves conflicts, derives changed-file coverage, and fills the Parent Reduction shape from `lanes/lane-schema.md`. In that shape, `parent` means this immediate review lead relative to a lane. The coordinator then makes final disposition and delivery routing from the lead's assessment.
