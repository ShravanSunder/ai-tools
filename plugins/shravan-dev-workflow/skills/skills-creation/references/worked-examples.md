...
4. Prove it: rerun as in step 1 against the fix. Completion: the rerun count and zero failures are reported, and any retry annotation the test carried is removed.

Quarantine is skip-with-ticket, and the ticket names the unguaranteed thing from step 2. Never quarantine by deleting the assertion or adding a retry.
```
...
- The spine is four steps because order changes behavior, and every completion criterion demands legwork: a captured log, a side-by-side quote, a rerun count.
- Exactly one branch exists, because only some runs need isolation mechanics; its predicate is observable from step 2's result, and it returns something the main path uses. That is the branch case of progressive disclosure — an attention decision.
