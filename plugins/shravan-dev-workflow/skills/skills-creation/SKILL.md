...
Emit the run note whenever the run dispatches review lanes, returns an evaluate verdict, edits skill files, or runs a proof route. A read-only accepted-spec verification or expiry stop does not need the run note unless one of those conditions also occurs. A run that does none of these is chat-only discussion and skips the ceremony.
