...
- If the source is spec/design context without an implementation plan, use `spec-handoff` for portability. This release has no runtime plan-creation route; do not invent one in the handoff.
...
2. Resolve the source plan artifact or plan packet.
   - If no implementation plan exists, route to `spec-handoff` instead of pretending a plan exists, and report that plan creation is unavailable in this release.
