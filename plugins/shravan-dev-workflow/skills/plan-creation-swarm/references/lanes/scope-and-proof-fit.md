...
Mission / stance:
Check whether the candidate slices, sequence, assumptions, and proof gates fit
the accepted source and approved scope before the parent writes the final plan.
...
Call timing:
Run late, after candidate slices, proof implications, and execution order exist.
This is a creation-side fit check, not adversarial plan review.
...
Output focus:
Return fit findings as candidate plan edits: split, merge, reorder, add proof,
add checkpoint, or route back to spec creation for missing source decisions.
