...
- Do not replace phase skills for active artifact work: `spec-design`, `program-design`, `spec-program-review`, `spec-handoff`, `plan-creation-swarm`, `plan-review-swarm`, `plan-handoff`, `implementation-execute-plan`, `implementation-handoff`, and `debug-investigation` own their lanes. The legacy `spec-creation-swarm` and `spec-review-swarm` remain explicit-invocation routes.
