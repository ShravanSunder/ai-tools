...
An implementation goal stays with the orchestrator, who owns design decisions with the user, the work-root, disposition, and the final report. A persistent implementation Sidekick owns planning, implementation, integration, proof, and corrections. An independent persistent review Sidekick examines implementation only after the orchestrator has assessed completed development.

## Execution Responsibility

Before delivery starts, use the current goal context or applicable handoff and the next phase's existing input/result contract. MUST load `manage-agents` to assign or resume the persistent implementation Sidekick and return its responsibility, continuity, and assignment boundary. The orchestrator keeps the existing work-root as `orchestrator`; the Sidekick joins it as `implementer`. The Sidekick may execute and prove directly or select bounded native Workers and standalone native Operators, each retained through its assignment corrections. For Router delivery, use `agent-collaboration`.
