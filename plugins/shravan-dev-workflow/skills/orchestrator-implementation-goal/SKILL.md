...
An implementation goal stays with the orchestrator, who owns design decisions with the user, every governing design and implementation-plan artifact, the coordination root, cross-PR integration, disposition, and the final report. After a main-authored plan is ready, one persistent implementation Sidekick may own each useful planned PR assignment's implementation, integration, proof, and corrections. An independent persistent review Sidekick examines implementation only after the orchestrator has assessed completed development and proof.

## Execution Responsibility

Before planning, use the current goal context or applicable handoff and the next phase's existing input/result contract. The orchestrator loads `plan-implementation` itself and authors the plan. Only after that plan identifies useful PR assignments, MUST load `manage-agents` to assign or resume the corresponding persistent implementation Sidekicks and return each responsibility, continuity, assignment boundary, prerequisite, and exact execution root. The orchestrator keeps the coordination root plus every execution root as `orchestrator`; each Sidekick joins only its execution root as `implementer`. Sidekicks may execute and prove directly or select bounded native Workers and standalone native Operators, each retained through its assignment corrections. For Router delivery, use `agent-collaboration`; roots are linked through existing message references rather than a new hierarchy mechanism.
...
3. Commission one implementation Sidekick per useful planned PR assignment. Record real prerequisites and write collisions from the plan. Independent assignments may run concurrently on separate execution roots; dependent assignments wait until the orchestrator verifies their prerequisites. If implementation or fitting proof is incomplete, each implementer invokes `implement-plan` for its assigned scope, executing directly or assigning a bounded native Worker under `manage-agents`. A milestone, completed slice, or phase return is a checkpoint.
...
- The orchestrator owns design, planning strategy, dependencies, PR grouping, and main assessment. Each implementer owns bounded implementation and fitting proof for its main-planned assignment. The review Sidekick owns independent review findings and coverage. The orchestrator owns source-backed disposition, cross-PR integration, routing, and the delivery terminal; the user owns material design decisions and phase gates. PR wrap-up owns PR gate inspection.
