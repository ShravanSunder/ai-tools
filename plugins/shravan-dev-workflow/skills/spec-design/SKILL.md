...
Each newly authored or substantively revised substantial Requirements and Specification document needs a meaningful visual beside the explanation it supports; reuse a current correct embedded visual when it still explains current meaning. Requirements imagery shows affected people, their job, current pain and desired outcome. Specification imagery keeps the system opaque and may show journeys, external context, domain entities or coverage without internal components, stores or enforcement. Use an available authorized Image Gen capability for a meaningful explanatory overview or journey instead of silently defaulting to Mermaid/table/text; an exact user format takes precedence, and no decorative filler satisfies the requirement. The shared rendering owner calls its generated-image procedure and returns any exact capability or meaning conflict; a cache path, prompt, chat-only image or broken embed is not completion. Generated imagery supplements normative prose/tables and any precise view whose fields it cannot preserve. A typo-only edit does not regenerate images, and a companion document's image does not satisfy this document's reader question.
...
- a newly authored or substantively revised substantial Requirements or Specification document lacks a shared-rendering result for either a meaningful current accepted embedded visual or the default available-authorized Image Gen route, or that result lacks an accepted project-local asset, resolving relative embed, semantic/readability inspection or required destination-preview result, without an exact user-format override or explicit owner-authorized deferral;
