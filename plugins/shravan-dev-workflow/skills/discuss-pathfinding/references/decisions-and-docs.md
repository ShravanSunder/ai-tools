...
IF a record is a durable handoff artifact whose ambiguity could change implementation ownership, behavior, or proof, reread only the record and the questions its real reader must answer. Answer those questions from the record without relying on the drafting discussion. Record `complete | partial | blocked` with the answers and exact ambiguity. Repair passages that produce wrong or hedged answers, then repeat this reader test until `complete`. A quick session's in-chat ledger does not trigger this; the predicate is the artifact's downstream weight.
