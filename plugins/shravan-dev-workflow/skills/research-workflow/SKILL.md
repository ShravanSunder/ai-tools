...
description: Use when a task needs source gathering, prior-art research, current docs or web evidence, memory or session-log mining, or saved-reader research before design, planning, review, or discussion can continue, and the evidence can be gathered one source class at a time. Not for extracting unwritten owner meaning (discuss-pathfinding) or repairing a shared model already held (discuss-clarify-mental-models).
---
...
6. Synthesize by question. Findings move from `lead` to `investigated`, then `accepted | refuted | unresolved`. Zero accepted findings is valid. State supported, refuted, complicated, and unresolved claims with their anchors, then recommend the next owning workflow.

For substantial research, MUST load `references/evidence-ledger.md` and write a repo-local ledger under `tmp/research-workflows/<date>-<slug>/`, unless the user asked for chat-only or no files or the tool surface cannot write. Name the exception in the result. Send progress after re-anchor, after source-class coverage, and before synthesis.

## Return

Return the questions, source-class coverage with verbatim queries and null results, artifact path or exception, verified claim classes and finding states, competing hypotheses, gaps, countercheck statuses when selected, and the recommended next workflow. Evidence for authoritative Why/What routes to `spec-design`; structural How to `program-design`; ready design planning to `plan-implementation`; ready plan execution to `implement-plan`; independent implementation judgment to `implementation-review` or `skills-creation` for runtime skill packages.
