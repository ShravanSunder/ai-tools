...
4. Admit reviewed-design and admitted-repository-improvement governing-basis variants through their canonical fields. Reject missing, stale, malformed, plan-only, mismatched, wrong-origin, or unproven authority without inference. When no plan record exists at all, the only admissible classes are `diff-only-limited` (the user explicitly asked for a diff-only review and no risk trigger is present) or `risk-triggered` with the user's confirmed goal statement recorded as the sole rail; both return with `source-backed readiness: not claimable`, and neither licenses inferring a plan or design from the PR description.
...
2. Read the complete governing basis and the complete base-to-reviewed diff yourself before composing anything, and materialize that diff to a file in system tmp so every reviewer receives the same content rather than a range it may not be able to expand. Every target file the coordinator or any reviewer loads is read completely before substantive judgment — never "enough to establish scope."
3. Collect the shared conceptual context every reviewer will receive, in citable form: the governing basis recorded by the canonical plan (the Requirements/Specification/Program Design set, or the admitted-finding basis evidence) as absolute paths, the confirmed goal boundary as quoted in/out statements with their authority source, accepted obligations with section anchors, constraints and non-goals each with the authority that imposed them, proof claims, and steering anchors. Owner meaning that exists only in chat is copied verbatim into the packet — a reviewer with no history cannot follow a pointer into a conversation.
...
MUST load `references/lanes/lane-schema.md` and return the filled shared packet and result envelope for every dispatch below. Every dispatch resolves its runtime through `manage-agents` as a fresh-context, single-assignment Delegate: no parent conversation history (never a self-fork), candidate-only authority, and read-only workspace access — reviewers never edit. The packet's `lane instructions` field carries the absolute paths of every reference the bullets below say the subagent loads, or their inlined text when the runtime cannot read the plugin cache; a reviewer dispatched into the reviewed repo cannot resolve this skill's relative paths. Reviewers may keep a tmp scratchpad outside the reviewed worktree as working state; the worktree stays untouched.

IF the spec-compliance receipt returns `misread requirement` or `scope underdelivery`, skip the chunk fan-out and return `needs-revision` to the semantic owner — an intent failure bounds every downstream finding.
...
- IF the review carries proof claims, dispatch `proof-challenge` using the shared packet plus the claim inventory. Subagent loads `references/lanes/lane-schema.md` and `references/lanes/proof-challenge.md`. Parallel-safe with chunk reviewers once the claim inventory is collected. Instance authority adds one named grant recorded in the packet — executing exactly the proof commands listed there, output confined to its tmp scratchpad — and stays otherwise read-only and candidate-only; that lane reference owns the execution boundary, including its pre-run write-set check, and may never widen into edits. Return one row per claim in this exact shape, plus every command run verbatim; the coordinator accepts the receipt only when no row has an empty field, verifies each command ran inside the grant, and reduces it:
  ...
  ```text
  claim | preflight write-set: scratchpad-only|ignored-build-artifacts|tracked-worktree|unknown | command run or challenge | observed vs claimed | false-green check: <how this could pass with the behavior absent> | exit status | rerun comparison: <both outcomes> or no failure
  ```
...
The rails are not optional context: before accepting any finding, the coordinator re-anchors it to a confirmed requirement, Specification obligation, Program Design element, or goal-boundary field, and tests whether the confirmed obligations still hold without the questioned mechanism. A reviewer proposal that adds unrequested scope is rejected as scope expansion; an unrequested element the diff already delivers gets removal or an owner `decision-needed`, never "well built" — reviewer agreement is not evidence, and rubber-stamping reviewer output is the failure this skill exists to prevent. A finding that breaks a load-bearing mental model stops and returns to the user with the failed assumption, evidence, and consequence.
...
- an accepted finding silently expands the confirmed goal, adds an unrequested subsystem, or weakens a requirement instead of being rejected as scope expansion;
- a delivered item that dispel mapped `absent`, or that spec-compliance classified `extra` or `scope overreach`, lacks either an accepted removal finding or a `decision-needed` return — "well built" is not a disposition;
