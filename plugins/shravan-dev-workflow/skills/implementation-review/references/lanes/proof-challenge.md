...
- before every execution, resolve what the command actually runs — the script it names, its pre/post hooks, and the relevant tool configuration — and classify its write set as `scratchpad-only | worktree | unknown`; `worktree` and `unknown` stop before execution with the predicted writes and evidence. Running once to discover behavior is forbidden;
