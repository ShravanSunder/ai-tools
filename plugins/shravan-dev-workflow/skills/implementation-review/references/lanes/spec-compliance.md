...
Expected inputs: the current review context; the complete governing-basis artifacts and the complete diff, not summaries.

Prerequisites: the admitted canonical plan record, governing basis, and diff range are complete and inspectable.

Check boundary: read-only review. Read-only discovery commands and a tmp scratchpad only; no proof-generation commands, edits, or workflow decisions.
