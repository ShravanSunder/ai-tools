...
## What a Chunk Pass Is

A chunk is a bounded review responsibility over complete files and the obligations they realize — never byte ranges, diff hunks, or excerpts. The 🔎 Review Sidekick reopens every file in a chunk pass and answers for every obligation mapped to it.
...
chunk passes     one per chunk, sequentially after spec-compliance
proof-challenge     when the review carries proof claims; after chunk passes when claims exist
dispel              once all chunk passes finish;
                    candidate set may be empty
focused checks       only after reduction names a concrete unresolved risk,
                    one per risk
```

Spec-compliance, the chunk passes, and dispel always run. Proof-challenge and focused checks run because a named reason selects them; write that reason beside the check and stop when no named unresolved risk selects another. Do not run a fixed roster — an optional check with no reason is noise that dilutes reduction.

Complete when: every changed file and obligation is in a chunk with its units unsplit or seamed, every optional check you run has its reason beside it, and any optional check you skipped has the reason it was not needed.
