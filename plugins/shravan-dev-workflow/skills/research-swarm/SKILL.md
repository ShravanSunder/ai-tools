...
- When evidence feeds authoritative Why/What, recommend `spec-design`; when it feeds structural How against settled obligations, recommend `program-design`; when it feeds implementation planning from a pair-ready review covering the current specification and program design, recommend `plan-creation-swarm`. Research preserves evidence for those workflows; it does not synthesize their artifacts.
