...
🐒 Sidekicks, 🔎 Review Sidekicks, and 🦉 Advisors run in separate persistent top-level conversations; 🛠️ Workers and 🔧 Operators run as native subagents. Reuse a persistent relationship's exact address across follow-ups and corrections. Persistence takes precedence over native availability: a native child never forms a persistent Sidekick. When the host's native page has no model for the role, use agent-router with Luna; no native stand-in.

**Resolve the exact model id.** Skills name lineage and effort, never ids. (1) If `~/.config/agent-context/model-map.md` exists and has a row for this host and role, use that id. (2) Otherwise pick from the host's live advertised list only an id of the current generation of the named lineage at the named effort: GPT-6 (Luna, Sol, Astra), Claude Opus 5.5, Claude Fable 5.x, or xAI Grok 4.6 or newer. Older generations (for example `gpt-5.x`, Codex 5.3, Opus 5, Sonnet 4.x) and `-fast` variants never qualify. (3) If nothing matches, use agent-router with Luna for 🛠️ Worker or 🔧 Operator work, or report the gap for a Frontier role. Record the chosen id in the receipt or ledger. Do not replace an adequate native assignment with a CLI wrapper to invent access flags.
...
For each accepted planned PR assignment, create or reuse one named persistent implementation Sidekick through a capability-verified route. Its commission carries the Main-authored plan and PR scope, accepted direction, sources, worktree, work references, execution authority, proof expectations, real prerequisites, and the route for design or plan questions. The commission also grants decide-and-record and stand-in authority inside its scope: the 🐒 Sidekick decides reversible in-scope calls and puts a contract-honoring stand-in at a named boundary for a missing piece, records each through `practices-show-me-your-work`, and keeps working instead of returning for every gap. It still returns anything that would change a public contract, persisted data format, or ownership. Its receipts list decisions made and open stand-ins. Seats and roots follow `practices-collaboration`: Main holds `orchestrator` on the coordination root and on each execution root, and the Sidekick joins its own execution root as `implementer` with `--actor self`. Setup is complete only after the Participant row on that exact execution root shows the role. Several independent PR assignments may run as several Sidekicks; dependent work waits for verified prerequisites.
...
**History.** A 🔎 Review Sidekick receives no inherited author or designer conversation history, on any host; self-fork and resume-self are full-history inheritance and are forbidden for it. "It already has all the context" is the rationalization; the context is the contamination. Its brief carries absolute source paths or inlined governing content for everything it will cite, owner meaning copied verbatim, and it loads its owning review workflow before walking the checks. Non-reviewers use the host's advertised history encoding by cost and benefit, including only what the stop condition depends on. ACPX assignments record `history none`.
...
Before closing an assignment, verify stable identity where continuity applies, declared authority and path scope, current source or head evidence for its claims, decisions made and open stand-ins, and any unresolved transport, provider, or proof gaps. Main reduces those checks into the overall verdict.
