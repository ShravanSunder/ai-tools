...
Name separately addressable agents purposefully as `<emoji> <role> · <purpose/>`, with a concrete purpose substituted. Do not name the main driver. For a persistent session, use a supported rename/display operation and verify its returned identity and visible name; its exact SessionRef remains separate. A ledger-only label does not prove that the UI title changed. If the host cannot name it, report that capability gap without blocking otherwise useful work. Names do not replace session addresses or grant authority.
...
- **Work:** One bounded research, implementation, drafting, reasoning, or analysis assignment, including its corrections and associated proof. Independent review belongs to a Review Sidekick.
