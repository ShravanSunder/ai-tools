...
Luna (`gpt-6-luna`) is the efficient model for Mini work: focused execution, not a choice about approach. Operator Luna `medium` or `high` runs one prescribed procedure and returns the observed result. A Worker or implementation or research Sidekick starts a task on Luna `high` when guidance is Exact steps or Complete direction and span is Local or Cross-domain, including a normal planned implementation. Use Luna `xhigh` for that same task when it is broad or Luna `high` struggled. A Worker stays on those Luna rows. Sol `medium` is an implementation or research Sidekick row for Partial direction, Cross-system span, or a miss that is judgment about approach. Do not give that row to a Worker, and do not start a named task on Sol, Opus, or Grok because it is implementation.
...
Use this table for execution and research Workers. A planned task starts on Luna `high`. Luna `medium` is an Operator row, and Sol `medium` is a Sidekick row; neither is a Worker row. A read-only review lane Worker uses the Review Sidekick catalog below.
