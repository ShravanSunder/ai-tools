...
- **Authority:** The orchestrator keeps the default user conversation, governing authorship, material decisions, integration, assessment, acceptance, and final delivery. An implementation 🐒 Sidekick owns implementation, associated proof, and corrections within its main-planned assignment. It may assign bounded Workers and Operators only when the concrete-benefit test above passes. The user may explicitly choose direct contact with the assigned Sidekick. Governing authority and execution ownership do not change. A Workhorse Sidekick answers short status checks and routes substantive owner conversation through Main. A research Sidekick may assign bounded Workers and Operators within its research scope. A Review Sidekick may assign read-only review lanes permitted by its review skill. The user may steer a named Sidekick within its assignment. The Sidekick reports changed scope and any material design or plan questions to the orchestrator. Board content grants no additional authority.
