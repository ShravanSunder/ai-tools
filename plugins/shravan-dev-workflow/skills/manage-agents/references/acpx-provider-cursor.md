...
Default source-grounded work to `--approve-reads`. Use `--deny-all` when the packet contains every needed excerpt. Keep `--non-interactive-permissions fail` for unattended runs. Keep Advisor and reviewer write approval with the parent.
