...
A newly authored or substantively revised substantial Program Design carries a reader-useful visual overview beside the structural explanation. Use an available authorized Image Gen capability for a meaningful explanatory overview instead of silently defaulting to Mermaid/table/text; an exact user format takes precedence, and no decorative filler satisfies the requirement. Route the main-authored composition through the shared rendering owner and keep the accepted durable embed alongside the required exact component, call, state, failure and proof views. An attractive generated overview with an invented owner, edge, state or boundary fails; a generated image never excuses a missing required field or becomes the authoritative design. Reuse only after freshness inspection, and do not regenerate for typo-only edits.
...
Do not present unverified Mermaid as the readable result. In a read-only or chat-only response, directly visible fenced plain text is an inspectable fallback: use it when Mermaid cannot be rendered and inspected and it can preserve the selected fields. Return the exact visual gap only when no inspectable fallback can preserve the relationship.
...
- a newly authored or substantively revised substantial Program Design lacks a shared-rendering result for either a meaningful current accepted embedded visual or the default available-authorized Image Gen route, or that result is missing, broken, uninspected, unreadable, inconsistent with exact structural views, or not accepted by the main, without an exact user-format override or explicit owner-authorized deferral;
