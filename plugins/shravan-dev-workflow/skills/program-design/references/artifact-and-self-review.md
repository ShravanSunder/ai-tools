...
Removal is valid only when it changes no human confirmation, correction, decision, trace, failure simulation, proof path, accepted-requirements coverage, or later authoritative lookup. Remove pass-through components, duplicate owners, unused seams, premature concurrency, process/review/PR narration, repeated companion ownership recitals, obscure headings, summaries that merely repeat the preceding model, and views that add no decision clarity. Preserve authoritative provenance and negative space; owners, interfaces, state, calls/flows, failure/recovery, trust, cutover, and proof decisions; and accepted debt with payer and revisit signal. After a deletion or simplification pass, report accepted-requirements coverage as one compact row per stable identity or accepted group with `covered | owner-authorized supersession | gap` plus its anchor, reusing the existing coverage view; a bare "coverage intact" assertion is not a report.
