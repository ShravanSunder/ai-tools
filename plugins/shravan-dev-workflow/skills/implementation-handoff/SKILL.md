...
- Do not run reviewers automatically. For `pre-review` or post-remediation review, classify `general-domain | runtime-skill-package`, prepare the complete admission packet, and recommend exactly `implementation-review` for general-domain work or `skills-creation` for runtime skill-package work; never perform or launch review here.
