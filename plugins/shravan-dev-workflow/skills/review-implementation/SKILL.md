...
For `non-substantial`, return one current row per changed file with `path | base/reviewed/diff anchors | reviewed source/search scope | consumer search/result anchors | freshness | no-effect conclusion`, then stop before canonical-plan admission or reviewer dispatch. A classification summary or scattered coverage bullets do not replace this row. Never compute a file hash or digest for review identity.
