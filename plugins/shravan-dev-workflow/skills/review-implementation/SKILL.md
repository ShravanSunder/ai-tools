...
Completion: `non-substantial` returns its complete per-file evidence rows. Meaningful or blocked admission returns classification, governing identities, unchanged tuple, approval identity, base/reviewed identity, diff, proof boundary, and `admit | blocked-input` explicitly.

## Reconstruct And Judge

1. Build one reusable evidence core containing governing authority identities, the unchanged canonical tuple and approval identity, base and reviewed identities, exact diff range, changed files, repository instructions, proof claims/evidence, constraints, known gaps, applicable risk predicates, and prior-coverage freshness. Instantiate the shared packet schema for each dispatch with its own assignment identity and lane identity plus that evidence core.
