...
7. MUST load `../../shared-references/canonical-implementation-plan.md` to choose the proportional Markdown form and repository home, produce and validate the canonical tuple and result-specific payload, and return the tuple plus explicit approval absence. Write or revise the plan with originating planner `plan-implementation`, then stop.

Completion: one immutable path-addressed canonical Markdown plan exists with `draft | revision-requested | blocked`, every obligation maps to a slice and proof gate, necessary edges and integration gates are explicit, and the returned tuple identifies that completed plan without a document digest.
