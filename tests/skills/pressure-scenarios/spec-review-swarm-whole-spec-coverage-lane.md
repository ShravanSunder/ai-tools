...
expect_decision_regex: whole-spec-coverage|whole-spec coverage
expect_proof_regex: target spec/design artifact|primary artifact inputs|target artifact coverage
expect_proof_regex: research ledger|source anchors and research|source/research artifacts
expect_proof_regex: mandatory.*whole-spec-coverage|first-class.*whole-spec-coverage
expect_proof_regex: product intent|requirements|technical contract|boundaries
expect_proof_regex: focused lanes.*do not replace|do not replace.*focused lanes|parent reducer
