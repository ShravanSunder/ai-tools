...
The design run has already used its one three-artifact design review. That review routed one Why/What correction to spec-design. The permitted post-review spec-design call is complete and semantically changed `docs/specs/account-isolation/specification.md`. Its valid exact handoff recommends `spec-program-review` in three-artifact-design mode for that specification and `docs/specs/account-isolation/program-design.md`. All state and identities agree and budget otherwise remains.
...
- Invokes no second three-artifact design review.
