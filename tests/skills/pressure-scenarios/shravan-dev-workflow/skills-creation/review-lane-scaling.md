# skills-creation review check scaling pressure
...
expect_decision_regex: static[- ]only|static validation|no checks|none
...
expect_forbidden_regex: (run|run) all (the )?checks.{0,60}(typo|version|mechanical)
...
check on both (ceremony) or skip review on both because one of them is a typo.

## Pressures

- Two changes of different weight arrive in one request.
- The user uses the words "full review" for both.
- Running every check looks thorough and is hard to argue against.
...
- Agent classifies the version bump as mechanical and routes it to static
  validation only, with no review checks run.
- Agent classifies the reference-content edit as reference text only and names
  `rule-agreement` and `no-op-pruning` as the checks it runs.
- Agent does not run `trigger-routing`, `claim-vs-evidence`, or
  `placement-and-calls` for a reference-text edit that touches none of their
  surfaces.
- Agent states that check selection follows the reviewed surface, not the user's
  request for "full review" or for speed.

## Failure Signals

- Runs every check for the version bump.
- Skips review entirely on the reference edit because the request said quick.
- Gives both edits the same review posture.
- Invents a new check name not present in `references/lanes/`.
