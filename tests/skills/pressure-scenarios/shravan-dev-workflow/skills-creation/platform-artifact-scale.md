...
expect_decision_regex: classification:\s*update|(run summary|artifact|state).{0,300}(compact|terse|small|scale|ceremony)
expect_proof_regex: platform.{0,3}mechanics(\.md)?
expect_proof_regex: codex.{0,160}(metadata|validation|marketplace|mechanics)
expect_proof_regex: (client-specific|platform-specific).{0,200}(invocation|control|policy|encoding)
...
expect_proof_regex: (installed[- ]cache|cache refresh|home).{0,180}(defer|not.{0,40}proof|not.{0,40}routine|not part|explicit|requires explicit|did not claim)|(defer|explicit).{0,180}(installed[- ]cache|cache refresh|home)
expect_proof_regex: classification:\s*update[\s\S]{0,1200}(proof:|shipping:|outcome:)
