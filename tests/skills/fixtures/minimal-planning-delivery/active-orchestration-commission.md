...
- `retired-skills/orchestrator-implementation-goal/SKILL.retired.md` contains valuable first-unproven-gate routing and terminal-condition discipline, but duplicates lifecycle state through `goal_id`, `details.md`, `events.jsonl`, transition-writer precedence, and elaborate closeout contracts.
...
orchestrator-implementation-goal selects and verifies these transitions.
...
## Run 4: `orchestrator-implementation-goal`
...
- A direct one-phase request bypasses `orchestrator-implementation-goal`. A phase-owned correction returned by review goes directly to that named owner rather than restarting a full design cycle.
