...
const executionGraphPreflightReceipt = {
...
async function writeReceiptFixture(
  receiptPath: string,
  receipt: unknown,
): Promise<{ receiptPath: string; receiptDigest: string }> {
...
function claimedRequirements(
  ...
  const knownRequirementIds = claimedRequirementIds.filter(
    (requirementId) => !unknownRequirementIds.includes(requirementId),
  );
  const calibratedGateRequirementIds = claimedRequirementIds.filter(
    (requirementId) => !untracedRequirementIds.includes(requirementId),
  );
...
function summary(
  ...
  return {
    ...
    claimedRequirementManifestDigest:
      props.claimedRequirementManifestDigest ?? claims.manifestDigest,
    parentAcceptanceReceiptDigest:
      "parentAcceptanceReceiptDigest" in props
        ? (props.parentAcceptanceReceiptDigest ?? null)
        : props.releaseAuthority === true
          ? `sha256:${"d".repeat(64)}`
          : null,
    parentAcceptanceSourceReceipt:
      props.parentAcceptanceSourceReceipt ??
      (props.releaseAuthority === true
        ? {
            receiptPath:
              "tests/test-utils/skill-pressure/config/authority-receipts/acceptance.json",
            receiptDigest: `sha256:${"f".repeat(64)}`,
          }
        : null),
    calibrationSourceReceipt:
      props.calibrationSourceReceipt ??
  ...
  return Promise.all(
    summaries.map(async (candidate, index) => {
      ...
      const calibrationSource =
        calibration === null ? null : `${JSON.stringify(calibration.receipt, null, 2)}\n`;
      ...
      if (calibrationSource !== null) {
        if (options.linkCalibrationSource === true) {
          const externalPath = path.join(
            repositoryRoot,
            `${candidate.scenarioId}-${fixtureSequence}-external-calibration.json`,
          );
          ...
          await writeFile(path.join(repositoryRoot, calibrationPath), calibrationSource, {
            flag: "wx",
          });
        }
      }
      ...
      const persistedAttemptReceipts =
        options.duplicateAttemptReceipt === true
          ? [...attemptReceipts.slice(0, -1), attemptReceipts[0]!]
          : attemptReceipts;
      const progressReceipt = await writeReceiptFixture(
        path.join(receiptDirectory, "progress-1.json"),
        {
          schemaVersion: 1,
          scenarioId: candidate.scenarioId,
          status: "completed",
      ...
      const executionPreflight = await writeReceiptFixture(
        path.join(receiptDirectory, "execution-graph-preflight.json"),
        {
          schemaVersion: 1,
          scenarioId: candidate.scenarioId,
      ...
      const acceptance =
        candidate.releaseAuthority &&
        candidate.parentAcceptanceReceiptDigest !== null &&
        calibration !== null
          ? createRunAcceptanceFixture({
              calibration,
              runDigest,
      ...
      const receipt = {
        ...
        authoritySnapshot: {
          evaluationRole: candidate.evaluationRole,
          freshness:
            options.registryFreshness ??
            (candidate.calibrationStatus === "calibrated" ? "fresh" : candidate.calibrationStatus),
          registrySnapshotDigest: candidate.registrySnapshotDigest,
      ...
      return validateV3ScenarioExecutionForAggregate({
        ...
        registryRow: {
          ...
          validityReview: {
            receiptPath: "fixture-validity.json",
            receiptDigest: fixtureAuthorityDigest("8"),
          },
          calibrationReceipt:
            calibrationSourceDigest === null
              ? null
...
describe("skill pressure aggregate receipt", () => {
  it("reports exact v3 authority and outcome counts for an incomplete diagnostic run", async () => {
    ...
    const receipt = createV3AggregateReceipt({
      ...
      results: await validateSummaries(repositoryRoot, claims, [
        ...
        summary(
          { scenarioId: "fail", outcome: "behavior_fail", reasonCode: "treatment_behavior_failed" },
          claims,
        ),
        summary(
          { scenarioId: "inconclusive", outcome: "inconclusive", reasonCode: "mixed_treatment" },
          claims,
        ),
        summary(
          { scenarioId: "not-evaluated", outcome: "not_evaluated", reasonCode: "missing_evidence" },
          claims,
        ),
  ...
  it("rejects a selected aggregate without a complete suite preflight reference", () => {
    const base = {
      runId: "missing-preflight",
      suiteKind: "diagnostic" as const,
      discoveredScenarioCount: 1,
      selectedScenarioIds: ["scenario"],
      claimedRequirements: claimedRequirements(["fixture-behavior"]),
      registrySnapshotDigest,
    ...
    expect(() =>
      createV3AggregateReceipt({ ...base, executionGraphPreflightReceipt: null }),
    ).toThrow(/requires an execution-graph preflight/u);
    expect(() =>
      createV3AggregateReceipt({
        ...base,
        executionGraphPreflightReceipt: {
    ...
    ).toThrow(/receipt path must be non-empty/u);
  });
  ...
  it("fails diagnostics for invalid, infrastructure, missing, or incomplete accounting", async () => {
    ...
    const scenarios = [
      {
        ...
        invalid: [
          { path: "invalid.md", reason: "scenario_contract" as const, detail: "invalid contract" },
        ],
    ...
    for (const scenario of scenarios) {
      ...
      expect(receipt.suite, scenario.name).toEqual({
        kind: "diagnostic",
        terminalState: "failed",
        success: false,
      });
  ...
  it("rejects summaries that were not validated from persisted authority receipts", () => {
    expect(() =>
      createV3AggregateReceipt({
        ...
        invalid: [],
        results: [
          summary({ scenarioId: "scenario" }),
        ] as unknown as readonly ValidatedV3ScenarioExecutionSummary[],
      }),
    ).toThrow(/not validated from persisted authority receipts/u);

    const prototypeForged = Object.create(
      summary({ scenarioId: "scenario" }),
    ) as ValidatedV3ScenarioExecutionSummary;
    expect(() =>
      createV3AggregateReceipt({
  ...
  it("freezes validated summaries so authority facts cannot change after branding", async () => {
    const repositoryRoot = await mkdtemp(path.join(tmpdir(), "aggregate-repository-"));
    const claims = claimedRequirements(["fixture-behavior"]);
    const [validated] = await validateSummaries(repositoryRoot, claims, [
  ...
  it("rejects linked authority sources and calibration receipts that differ from the registry row", async () => {
    ...
    await expect(
      validateSummaries(linkedRepository, claims, [gate], {
    ...
    await expect(
      validateSummaries(mismatchedRepository, claims, [gate], {
        registryCalibrationPath:
          "tests/test-utils/skill-pressure/config/authority-receipts/other-calibration.json",
      }),
  ...
  it("rejects release authority when a scenario claims same-run demotion", async () => {
    ...
    await expect(
      validateSummaries(repositoryRoot, claims, [
        summary(
          {
            scenarioId: "scenario",
            evaluationRole: "gate",
    ...
    ).rejects.toThrow(/demoted.*release authority/u);
  });
  ...
  it("rejects self-declared repetition counts and stale gate release authority", async () => {
    ...
    await expect(
      validateSummaries(countRepository, claims, [summary({ scenarioId: "scenario" }, claims)], {
  ...
  it("rejects persisted evidence and behavior trace mutations that do not match authority digests", async () => {
    ...
    await expect(
      validateSummaries(evidenceRepository, claims, [summary({ scenarioId: "scenario" }, claims)], {
  ...
  it("rejects execution accounting above its token cap and stale evidence after accounting changes", async () => {
    const claims = claimedRequirements(["fixture-behavior"]);
    const overCapRepository = await mkdtemp(path.join(tmpdir(), "aggregate-repository-"));
    await expect(
    ...
    ).rejects.toThrow(/accounting exceeds/u);

    const staleDigestRepository = await mkdtemp(path.join(tmpdir(), "aggregate-repository-"));
    await expect(
      validateSummaries(
        staleDigestRepository,
        claims,
        [summary({ scenarioId: "scenario" }, claims)],
        {
          receiptObservedAccounting: {
  ...
  it("rejects diagnostic release authority and release authority without a calibrated gate pass", async () => {
    ...
    await expect(
      validateSummaries(uncalibratedRepository, claims, [
