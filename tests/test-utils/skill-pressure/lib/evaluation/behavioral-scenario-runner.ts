...
import { runSubjectRepetition, type SubjectRepetitionReceipt } from "./subject-repetition.js";
import {
...
export async function executeBehavioralScenario(
  ...
  const disabledAmbientSkillPaths = [
    ...new Set([
  ...
  const executionCapTracker = createScenarioExecutionCapTracker({
  ...
  const executionGraphPreflight = await writeJsonReceipt({
    ...
    fileName: "execution-graph-preflight.json",
    receipt: {
      schemaVersion: 1,
  ...
  try {
    return await executeV3BehavioralScenario({
      contract,
      ...
      executionAccounting: {
        preflightReceipt: {
          receiptPath: executionGraphPreflight.receiptPath,
          receiptDigest: executionGraphPreflight.receiptDigest,
        },
        snapshot: executionCapTracker.snapshot,
      },
      ...
      executeSubjects: async (request) => {
        ...
        for (const [variant, receipts] of [
          ...
          for (const [index, receipt] of receipts.entries()) {
            ...
            if (repetition === undefined || attemptReceiptPath === undefined) {
              throw new Error(
                `selected repetition lacks its durable attempt binding: ${receipt.repetitionId}`,
              );
            }
        ...
        return {
          baseline: result.baseline.map((receipt) =>
            requiredRepetition(repetitionsById, receipt.repetitionId),
          ),
          treatment: result.treatment.map((receipt) =>
            requiredRepetition(repetitionsById, receipt.repetitionId),
          ),
        };
      },
      executeSemanticReview: async ({ packet, signal }) =>
    ...
    await writeJsonReceipt({
      receiptDirectory: executionReceiptDirectory,
      fileName: "execution-accounting.json",
      receipt: {
        schemaVersion: 1,
        scenarioId: contract.scenarioId,
        executionGraph: props.executionBudget.executionGraph,
        acceptedCaps: props.executionBudget.acceptedCaps,
        observed: executionCapTracker.snapshot(),
      },
      secrets: redactionSecrets,
    });
  }
