...
export function selectV3SuiteScenarios(
  ...
  for (const candidate of props.candidates) {
    if (!Number.isSafeInteger(candidate.repetitions) || candidate.repetitions < 5) {
      throw new Error(`selection candidate ${candidate.scenarioId} must require at least five repetitions`);
    }
  }
